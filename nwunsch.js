...
function NeedlemanWunsch(script1, script2, pmatch, pmis, pgap) {
	...
	this.align = function(){
		...
		this.cost[0][0] = 0.;
		for (var i=1; i <= this.length1; i++) {this.cost[i][0] = this.cost[i-1][0] + this.pmis;}
		for (var j=1; j <= this.length2; j++) {this.cost[0][j] = this.cost[0][j-1] + this.pmis;}
		// propagate this.cost
		var top, left, diag;
		for (var i = 1; i <= this.length1; i++) {
			for (var j = 1; j <= this.length2; j++) {
				...
				/*
				 * Store direction from which score was assigned
				 * When there are two ways that have the same score, 
				 * we assign diagonal the highest priority, then top and then left.
				 */
	...
	this.makeCostTable = function() {
		...
		for (var i = 0; i <= this.length1; i++) {
			...
			for (var j = 0; j <= this.length2; j++) {
				var d = (this.directions[i][j] == undefined ?  '' : this.directions[i][j]);
	...
	/**
	 * Returns correspondence between two lists.
	 * Call after alignment is computed using align().
	...
	this.getMatchArrays = function() {
		...
		while (i > 0 || j > 0) {
			 ...
			 else if (this.directions[i][j] == 'l') { // text2[j-1] corresponds to a gap in text1
				 corr2[j-1] = -1;
				 j--;
			 }
			 else if (this.directions[i][j] == 't') { // text1[i-1] corresponds to a gap in text2
				 corr1[i-1] = -1;
				 i--;
			 } 
			 else if (this.directions[i][j] == 'd') { // text1[i-1] corresponds to text2[j-1]
				 corr1[i-1] = j-1;
				 corr2[j-1] = i-1;
				 i--;
				 j--;
	...
	this.traceback = function(){
		 ...
		 while (i > 0 || j > 0) {
			 ...
			 else if (this.directions[i][j] == 'l') { // move from left: gap in text 1
				 text1_align.push('-');
				 text2_align.push(text2[j-1]);
				 j--;
			 }
			 else if (this.directions[i][j] == 't') { // move from top: gap in text 2
				 text1_align.push(text1[i-1]);
				 text2_align.push('-');
				 i--;
			 } 
